...
class gather_man {
	...
	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
        if( time() - $this->game_map_timer > 120 ) {
	...
	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
    {
        foreach( $this->players as $k => $player ) {
            if( $player == $nick ) {
                ...
                $this->player_color[$n_nick] = $this->player_color[$name];
                unset( $this->player_color[$name] );
                $this->player_rank[$n_nick] = $this->player_rank[$name];
                unset( $this->player_rank[$name] );
	...
	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
    {
        $this->refresh = $this->game_server->get_refreshx();

        $this->game_alpha_score = $this->refresh['team'][1];
        $this->game_bravo_score = $this->refresh['team'][2];
    }
	...
	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
    {
        ...
        if( $this->game_active == false && $this->is_game_full() ) {
            ...
            if( $this->full_map() ) {
                ...
                $result = $this->game_server->get_tag_string() . " ~ " .  sprintf("%-13s",$this->refresh['map']) . " has finished after " . $timer . "min.";
                $result .= " With score:". BOLD . RED . " " . sprintf('%02d',$this->refresh['team'][1]) . BOLD . MCOLOR;
                $result .= " -" . BLUE . BOLD . " " . sprintf('%02d',$this->refresh['team'][2]) . MCOLOR;   
                
                $this->rounds[] = $this->refresh;
            }
        }
        $this->refresh = $this->game_server->get_refreshx();
        ...
        $this->game_map_timer = time();
        $this->game_map = $this->refresh['map'];

        return $result;
    }

	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
    public function player_joined( $name, $hwid = null) {
	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
        if( array_key_exists($name, $this->game_players) ) return;

        $this->game_players[$name] = $hwid;
        $this->game_pc++;
	...
	/* ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */    
        ...
        if( $this->game_active ) {
            ...
            if( $this->is_game_empty() ) {
                ...
                return $this->game_server->get_tag_string() . " ~ Gather finished";
            }
        }
